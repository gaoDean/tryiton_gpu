...
REF_LABEL_MAP = {"upper": 0, "lower": 1, "overall": 2, "shoe": 3, "bag": 4}
...
class FastFitPipeline:
    ...
    def __call__(
        ...
        ref_images = [
            prepare_image(image, self.device, self.weight_dtype) for image in ref_images
        ]
        if pose is not None:
            pose = prepare_image(
                pose, self.device, self.weight_dtype, do_normalize=False
            )
        masked_person = (
            person * (1 - mask) + mask * pose
            if pose is not None
            else person * (1 - mask)
        )
        ...
        masked_person_latent = (
            self.vae.encode(masked_person).latent_dist.sample()
        ...
        if do_classifier_free_guidance:
            ...
            ref_images_latent = [
                torch.cat([torch.zeros_like(image), image])
                for image in ref_images_latent
            ]
            if ref_attention_masks is not None:
                ref_attention_masks = [
                    torch.tensor([ref_attn_mask] * 2).to(self.device)
                    for ref_attn_mask in ref_attention_masks
                ]
            if ref_labels is not None:
                ref_labels = [
                    torch.tensor([ref_label] * 2).to(self.device)
                    for ref_label in ref_labels
                ]
        else:
            if ref_attention_masks is not None:
                ref_attention_masks = [
                    torch.tensor([ref_attn_mask]).to(self.device)
                    for ref_attn_mask in ref_attention_masks
                ]
            if ref_labels is not None:
                ref_labels = [
                    torch.tensor([ref_label]).to(self.device)
                    for ref_label in ref_labels
                ]

        # Denoising loop
        extra_step_kwargs = prepare_extra_step_kwargs(
            self.noise_scheduler, generator, eta
        )
        num_warmup_steps = (
        ...
        # Cache Ref KV
        for ref_image_latent, ref_label, ref_attention_mask in zip(
            ref_images_latent, ref_labels, ref_attention_masks
        ):
            ...
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    ...
                    attention_mask=torch.tensor(
                        [1] * inpainting_latent_model_input.shape[0]
                    ).to(self.device),
                )[0]
        ...
        # VAE Decoding
        latents = (1 / self.vae.config.scaling_factor * latents).to(
